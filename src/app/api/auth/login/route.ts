...
export const POST = async (req: NextRequest) => {
  ...
  if (!JWT_SECRET_KEY) {
    console.error("❌ [API] Authentication failed: JWT_SECRET_KEY is not defined.");
  ...
  try {
    ...
    if (!email || !password) {
      console.warn('[API] Login failed: Email or password not provided.');
    ...
    if (!user || !user.password) {
      console.warn(`[API] Login failed: User not found or no password set for ${email}.`);
    ...
    if (!passwordMatch) {
      console.warn(`[API] ❌ Password mismatch for user ${user.username}.`);
      return NextResponse.json({ message: 'Invalid credentials' }, { status: 401 });
    }
